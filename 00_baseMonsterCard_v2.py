...
def delete_creature(creature):
    while True:
        ...
        if creature.lower() in [name.lower() for name in creature]:
            creature.remove([name for name in creature if name.lower() == creature.lower()][0])  # remove the first instance of the creature
            easygui.msgbox(f"{creature} has been deleted from the creature list.", title="Delete Creature")
            return creature  # return the updated list
        else:
